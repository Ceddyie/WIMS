...
export class ProductSelectionComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.webSocketAPI = new WebSocketAPI(new ProductSelectionComponent(this.http, this.formBuilder, this._snackBar, this._matDialog, this.router, this.productService))
    this.connect();
  }
  ngOnDestroy() {
  ...
  private getAllProducts() {
    this.productService.getAllProducts().subscribe((response) => {
      ...
      if (this.products.imageUrl.isEmpty()) {
      }
    },
        (error) => {
