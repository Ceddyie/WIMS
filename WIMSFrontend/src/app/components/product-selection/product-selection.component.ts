...
export class ProductSelectionComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.webSocketAPI = new WsPsAPI(new ProductSelectionComponent(this.http, this.formBuilder, this._snackBar, this._matDialog, this.router, this.productService, this.route))
    this.connect();
  }
  ngOnDestroy() {
